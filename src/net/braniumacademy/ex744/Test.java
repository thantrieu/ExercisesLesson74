package net.braniumacademy.ex744;

import net.braniumacademy.ex744.comparator.*;
import net.braniumacademy.ex744.exceptions.*;
...
public class Test {
    public static void main(String[] args) {
        ...
        do {
            ...
            out.println("3. Tìm nhân viên theo tên gần đúng");
            out.println("4. Tìm nhân viên theo mã gần đúng");
            out.println("5. Tìm nhân viên có mức lương >= x");
            out.println("6. Tìm giám đốc theo nhiệm kì");
            out.println("7. Sửa lương nhân viên theo mã");
            out.println("8. Xóa nhân viên theo mã cho trước");
            out.println("9. Tính lương nhân viên");
            out.println("10. Tính thưởng nhân viên");
            out.println("11. Sắp xếp danh sách nhân viên");
            out.println("12. Hiển thị mức thưởng và cách nhận thưởng");
            out.println("13. Hiển thị bảng lương");
            out.println("14. Lưu danh sách nhân viên, giám đốc");
            ...
            switch (choice) {
                ...
                case 4:
                    if (employees.size() > 0) {
                        var id = "";
                        out.println("Nhập mã nhân viên cần tìm: ");
                        id = input.nextLine().trim(); // nhập mã nhân viên
                        var result = searchById(employees, id);
                        if (result.size() > 0) {
                            showMessage("Tìm thấy " + result.size()
                                    + " kết quả");
                            showEmployees(result);
                            ...
                            out.println(
                                    "==> Không tìm thấy nhân viên nào mã \""
                                            + id + "\"");
                        }
                    } else {
                        showMessage("Danh sách nhân viên rỗng");
                ...
                case 7:
                    if (employees.size() > 0) {
                        ...
                        Employee searchEmp = searchAnEmpById(employees, id);
                        if (searchEmp == null) {
                            showMessage("Không tồn tại nhân viên mã \"" + id + "\"");
                        } else {
                            out.println("Nhập mức lương mới: ");
                            var newSalary = Float.parseFloat(input.nextLine().trim());
    ...
    private static Employee searchAnEmpById(List<Employee> employees, String id) {
    ...
    /**
     * Phương thức tìm kiếm nhân viên theo mã gần đúng
     *
     * @param employees danh sách nhân viên cần tìm
     * @param id        mã của nhân viên cần tìm
     * @return danh sách nhân viên tìm được hoặc danh sách rỗng
     * nếu không tìm thấy nhân viên nào
     */
    private static List<Employee> searchById(List<Employee> employees, String id) {
        List<Employee> result = new ArrayList<>();
        for (var emp : employees) {
            // nếu có mã nhân viên trùng một phần
            if (emp.getEmpId().toLowerCase()
                    .matches(".*" + id.toLowerCase() + ".*")) {
                result.add(emp);
            }
        }
        return result;
     ...
     */
    private static void sortEmployees(List<Employee> employees, int option) {
        switch (option) {
            case 1:
     ...
     * @param employees danh sách nhân viên
     */
    private static void showPayroll(List<Employee> employees) {
     ...
     * @param employees danh sách nhân viên
     */
    private static void showBonus(List<Employee> employees) {
     ...
     * @param employees danh sách nhân viên
     */
    private static void calculBonus(List<Employee> employees) {
        for (int i = 0; i < employees.size(); i++) {
     ...
     * @param employees danh sách nhân viên
     */
    private static void calculSalary(List<Employee> employees) {
        for (int i = 0; i < employees.size(); i++) {
            var emp = employees.get(i);
     ...
     */
    private static List<Employee> findManagerByTerm(
            List<Employee> employees, int startYear, int endYear) {
        List<Employee> result = new ArrayList<>();
     ...
     */
    private static List<Employee> searchBySalary(
            List<Employee> employees, float salary) {
        List<Employee> ressult = new ArrayList<>();
        for (var emp : employees) {
     ...
     * @param employees danh sách nhân viên
     */
    private static void updateEmployeeId(List<Employee> employees) {
     ...
     */
    private static boolean writeEmpToFile(List<Employee> employees,
                                          String fileName) {
     ...
     */
    private static boolean removeEmp(List<Employee> employees, String id) {
        for (int i = 0; i < employees.size(); i++) {
    ...
    /**
     * Phương thức tìm nhân viên theo tên gần đúng
     * chỉ cần trong tên có cụm từ cần tìm là sẽ cho
     * kết quả
     *
     * @param employees danh sách nhân viên
     ...
     */
    private static List<Employee> searchByName(List<Employee> employees, String name) {
        List<Employee> res = new ArrayList<>();
        for (var emp : employees) {
            var firstName = emp.getFullName().getFirst();
            // nếu tên so sánh trùng nhau 1 phần
    ...
    private static void showEmployees(List<Employee> employees) {
        ...
        out.printf("%-15s%-25s%-15s%-15s%-20s%-15s%-15s%-15s%-15s%-15s\n",
    ...
    private static void showEmp(Employee emp, SimpleDateFormat dateFormat) {
        out.printf("%-15s%-25s%-15s%-15s%-20s" +
    ...
    private static void showManagers(List<Employee> managers) {
        if (managers.size() > 0) {
            var format = "dd/MM/yyyy";
            SimpleDateFormat dateFormat = new SimpleDateFormat(format);
    ...
    private static Manager createManager(Scanner input) {
        ...
        try {
            return new Manager(employee, start, end);
                ...
                | InvalidEmailException | InvalidIdentityNumberException e) {
            e.printStackTrace();
        }
        return null;
    }
